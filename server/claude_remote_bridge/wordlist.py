"""Word list for human-typeable bridge tokens.

A bridge token is a passphrase — a handful of short words joined by hyphens
(``coral-anvil-mango-scoop-visor``) — so it can be read off the startup banner
and typed into the panel's Settings card by hand without error. Contrast the
old 32-character random ``secrets.token_urlsafe`` blob. See
``bridge._generate_passphrase``.

Source: the EFF "Short Wordlist #1" (Joseph Bonneau, Electronic Frontier
Foundation, 2016 — https://www.eff.org/dice): 1296 short (3-5 letter),
memorable, mutually-unambiguous English words chosen expressly for typing
passphrases by hand. Licensed CC BY 3.0 US
(https://creativecommons.org/licenses/by/3.0/us/).

The list's single hyphenated entry ("yo-yo") is dropped so every word joins
cleanly on hyphens, leaving 1295 words (~10.34 bits of entropy each under a
uniform ``secrets.choice``).
"""
